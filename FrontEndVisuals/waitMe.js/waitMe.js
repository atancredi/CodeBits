...
//function that inits/toggles the waitMe on an element (atancredi)
function waitMeToggle(element_selector, options) {

	//detect if the waitMe on the element is going or not
	var isWaitMeGoing = element_selector.hasClass("waitMe_container");

	if (isWaitMeGoing) element_selector.waitMe("hide");
	else element_selector.waitMe(options);

}

//function that binds the creation and destruction of a waitMe based on event (atancredi)
function waitMeBind(element_selector, trigger_selector, event, options) {

	trigger_selector.on(event, function (e) {

		e.preventDefault();
		e.stopPropagation();

		waitMeToggle(element_selector, options);
